...
List<_HeartRateData> apiData = [];

class _chartScreenState extends State<hrchartScreen>
  ...
  String dateTime(String datefm) {
    ...
    var year = date.year + 543; // add 543 to the year
    return customFormat.format(DateTime(year, date.month, date.day, date.hour,
        date.minute, date.second, date.millisecond, date.microsecond));
  }

  String myFormat(String datefm) {
    var date = DateTime.parse(datefm);
    var customFormat = DateFormat("MMMM yyyy", "th_TH");
    return customFormat.format(date);
  }

  myinfo() {
    for (var data in dataList) {
      apiData.add(new _HeartRateData(dateToInt(data["date"], data["time"]),
          data["pulse"], 10.0, dateTime(data["date"]), data["time"]));
    }
  ...
  Future<http.StreamedResponse> fetchHeartRateData(String duration) async {
    ...
    var request = http.Request(
        'GET', Uri.parse('http://192.168.1.129:8000/get_hrfilter/'));
    switch (duration) {
      case "week":
...
class _HeartRateData {
  _HeartRateData(
      this.datetime, this.heartRate, this.radius, this.date, this.time);
  final time;
  final date;
  final int datetime;
  final int heartRate;
  ...
  static List<charts.Series<_HeartRateData, num>> _createSampleData() {
    return [
      new charts.Series<_HeartRateData, num>(
        id: 'Sales',
        colorFn: (_HeartRateData hr, _) {
          //final bucket = sales.heartRate / maxMeasure;

          if (hr.heartRate <= 60) {
            return charts.Color.fromHex(code: '#F8D277');
          } else if (hr.heartRate > 60 && hr.heartRate < 120) {
            return charts.Color.fromHex(code: '#61D2A4');
          } else {
            return charts.Color.fromHex(code: '#DD5571');
          }
        },
        domainFn: (_HeartRateData hr, _) => hr.datetime,
        measureFn: (_HeartRateData hr, _) => hr.heartRate,
        radiusPxFn: (_HeartRateData hr, _) => hr.radius,
        data: apiData,
      ),
    ];
  }
