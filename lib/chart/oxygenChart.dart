...
class oxgchartScreen extends StatefulWidget {
  final id_user;
  final id_patient;
  oxgchartScreen({this.id_user, this.id_patient});
  @override
  State<oxgchartScreen> createState() => _oxgchartScreenState();
}

List<Map<String, dynamic>> dataList = [];
List<_OxygenRateData> apiData = [];

class _oxgchartScreenState extends State<oxgchartScreen>
  ...
  myinfo() {
    for (var data in dataList) {
      apiData.add(new _OxygenRateData(dateToInt(data["date"], data["time"]),
          data["oxygen"], 10.0, dateTime(data["date"]), data["time"]));
    }
  ...
  Future<http.StreamedResponse> fetchHeartRateData(String duration) async {
    final url = 'http://192.168.1.163:8000/get_oxygenFilter';
    //final response = await http.get(Uri.parse(url));
    //print(response.statusCode);
    var headers = {'Content-Type': 'application/json'};

    var request =
        http.Request('GET', Uri.parse(baseUrl + '/get_oxygenFilter/'));
    switch (duration) {
      case "week":
        ...
        title: Row(
          ...
          children: [
            Container(
                ...
                height: 22,
                child:
                    Image.asset("assets/oxygen.png", color: Color(0xFF61D2A4))),
            Padding(
...
class _OxygenRateData {
  _OxygenRateData(
      this.datetime, this.oxygenRate, this.radius, this.date, this.time);
...
class SimpleScatterPlotChart extends StatelessWidget {
  final List<charts.Series<_OxygenRateData, num>> seriesList;
  final bool animate;
  ...
  static List<charts.Series<_OxygenRateData, num>> _createSampleData() {
    final random = Random();

    return [
      new charts.Series<_OxygenRateData, num>(
        id: 'Oxygen',
        colorFn: (_OxygenRateData oxy, _) {
          //final bucket = sales.oxygenRate / maxMeasure;

          if (oxy.oxygenRate <= 95) {
            return charts.Color.fromHex(code: '#F8D277');
          } else if (oxy.oxygenRate <= 90) {
            return charts.Color.fromHex(code: '#DD5571');
          } else {
            return charts.Color.fromHex(code: '#61D2A4');
          }
        },
        domainFn: (_OxygenRateData sales, _) => sales.datetime,
        measureFn: (_OxygenRateData sales, _) => sales.oxygenRate,
        radiusPxFn: (_OxygenRateData sales, _) => sales.radius,
        data: apiData,
      ),
    ];
  }
